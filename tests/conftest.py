...
def guard_futures_cleanup():
    ...
    orig_init = aio.future._Future.__init__

    def future_init(fut, *args, **kwargs):
        futures.add(fut)
        orig_init(fut, *args, **kwargs)

    with patch.object(aio.future._Future, "__init__", future_init), patch.object(
        aio.future._Future, "__hash__", lambda fut: id(fut)
    ):
        yield futures
